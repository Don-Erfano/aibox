...
import { defaultValues, smsSchema, smsSchemaType } from './schema';
import {
...
import { NotifMessage } from '@/components/pages/messages/public-message/add-public-message/interface';
...
const AddSmsPage: FC = () => {
  ...
  const form = useForm<smsSchemaType>({
  ...
  const onSubmit: SubmitHandler<smsSchemaType> = async (data) => {
    try {
      ...
      router.push(MESSAGES_ROUTES.SMS);
    } catch (error: any) {
  ...
  return (
    <FormContainer
      title={isEditMode ? strings.edit_new_sms : strings.add_new_sms_message}
    >
      <Form {...form}>
